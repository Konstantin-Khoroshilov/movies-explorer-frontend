...
function Profile({ navigationVisible, handleCloseClick, handleMenuClick, reqStatusMsg, handleUserUpdate, onLogout, loggedIn }) {
      ...
      <section className="profile">
        ...
        <form onSubmit={onSubmit} className="profile-form" noValidate>
          ...
          <input
            type="submit"
            className={
              emailIsValid && nameIsValid && (email !== currentUser.email || name !== currentUser.name)
                ? "profile-form__submit link"
                : "profile-form__submit disabled-button"
            }
            value="Редактировать"
            disabled={emailIsValid && nameIsValid && (email !== currentUser.email || name !== currentUser.name) ? false : true}
          />
        </form>
